...
Web ブラウザで http://vhdl.cottonrose.jp/sim/ を開き、vhdl20.vhd をダウンロードし、先ほどのフォルダにダウンロードします。
...
Web ブラウザで http://vhdl.cottonrose.jp/sim/ を開き、vhdl21.vhd をダウンロードし、先ほどのフォルダにダウンロードします。
...
プロセス文 div_proc が１秒を作る。それを利用してプロセス文 cnt_proc 内で signal c で０～９をカウントする。
...
.. todo:: タイミングチャートの読み方がわからない

先ほどの階層設計を応用して、10 進数 3 桁のカウンタを作り、シミュレータ上で動作を確認します。

D:\\Altera\\(学籍番号) に vhdl22 というフォルダを作ります。

Web ブラウザで http://vhdl.cottonrose.jp/sim/ を開き、vhdl22.vhd をダウンロードし、先ほどのフォルダにダウンロードします。
