...
この場合、MSB @<fn>{f41} が bit 6 、LSB @<fn>{f42} が bit 0 という宣言になります。ここに代入する値は、たとえば１０進数の１０であれば

//emlist[][vhdl]{
"0001010"
//}

というふうにダブルクオーテーションで括ります。左が bit6 、右が bit0 です。代入する値は、代入先の信号とビット幅が一致している必要があります @<fn>{f43} 。
...
//footnote[f41][変化することにより全体の値が大きく変化するビット]
//footnote[f42][変化から値全体の変化が一番小さいビット]
//footnote[f43][ビット幅が一致していなくてもツール上エラーにならない場合があり、発見しづらいバグになりやすいです。]
